    ...
    <Link to={`/bookDetails/${bookId}`}>
      <div className="card bg-base-300 w-90 shadow-xl p-5 mx-auto">
        <figure className="p-4 bg-gray-300 w-3/3 mx-auto">
          <img className="h-[160px]" src={image} alt="Shoes" />
        </figure>
        <div className="card-body">
          <div className="flex items-center gap-3">
            {tags.map((tag) => (
              <button className="btn rounded-3xl bg-gray-200 border-none text-[#23BE0A] font-semibold">
                {tag}
              </button>
            ))}
          </div>
          ...
          <p className="text-xl font-semibold">{publisher}</p>
